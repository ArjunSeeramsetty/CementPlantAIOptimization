"""
Temperature Profile Validator

Validates temperature consistency, gradients, and outliers for industrial process data.
"""
...
def validate_temperature_profile_consistency(
    df: pd.DataFrame,
    ...
    if not temp_columns:
        ...
        validation_results["issues_found"].append(
            "No temperature columns found for validation"
        )
    ...
    if len(temp_data) == 0:
        validation_results["issues_found"].append(
            "No complete temperature data available"
        )
        return validation_results
    ...
    for temp_col in temp_columns:
        ...
        temp_stats = {
            ...
            "cv": (
                float(temp_values.std() / temp_values.mean())
                if temp_values.mean() != 0
                else 0
            ),
        ...
        outliers = temp_values[
            (temp_values < lower_bound) | (temp_values > upper_bound)
        ]
        temp_stats["outlier_count"] = len(outliers)
        temp_stats["outlier_percentage"] = float(
            len(outliers) / len(temp_values) * 100
        )
    ...
    if len(temp_columns) > 1:
        ...
        for i, col1 in enumerate(temp_columns):
            for j, col2 in enumerate(temp_columns):
                if (
                    i < j
                    and col1 in temp_correlations.columns
                    and col2 in temp_correlations.columns
                ):
                    ...
                    if not np.isnan(corr_val) and abs(corr_val) < 0.5:  # Low correlation
                        correlation_issues.append(
                            f"{col1} vs {col2}: r={corr_val:.3f}"
                        )

        if correlation_issues:
            validation_results["issues_found"].extend(
                [
                    f"Low temperature correlation: {issue}"
    ...
    if time_column and time_column in df.columns:
        ...
        for temp_col in temp_columns:
            ...
            if gradient_violations > 0:
                validation_results["gradient_violations"] += int(
                    gradient_violations
                )
                validation_results["issues_found"].append(
    ...
    total_violations = (
        validation_results["profile_violations"]
        + validation_results["gradient_violations"]
    )
    if len(temp_data) == 0:
    ...
    if validation_results["profile_violations"] > 0:
        validation_results["recommendations"].append(
            "Investigate temperature sensor calibration"
        )
        validation_results["recommendations"].append(
    ...
    if validation_results["gradient_violations"] > 0:
        validation_results["recommendations"].append(
            "Check for temperature control system issues"
        )
